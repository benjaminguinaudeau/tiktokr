...
+ [Using tiktokr with Docker](https://github.com/benjaminguinaudeau/tiktokr#authentification)
...
TikTok requires API queries to be identified with a unique hash. To get this hash, `tiktokr` runs a `puppeteer-chrome` session in the background. Apparently `puppeteer` sometimes causes issues on some operating systems, so we also created a Docker image, that can be run on any computer with Docker installed. 
...
If you experience a problem try to install Docker as outlined in the steps below.
...
To run `tiktokr` with Docker, simply run `tk_auth()` with `docker = TRUE` which sets the necessary environment variable.
